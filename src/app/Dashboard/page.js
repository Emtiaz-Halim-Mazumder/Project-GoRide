...
export default function DashboardPage() {
  ...
  const myRides = rides.filter(r => r.creator?._id === user?._id || (!r.creator && activeTab === 'myRides'));
  const othersRides = rides.filter(r => r.creator?._id !== user?._id && r.creator);
  const currentRides = activeTab === 'myRides' ? myRides : othersRides;

  const ridesToShow = filterPrefs.length > 0
    ? currentRides.filter(r => Array.isArray(r.preferences) && filterPrefs.every(p => r.preferences.includes(p)))
    : currentRides;
  ...
  const handleUpdateRide = async (id) => {
    try {
      ...
      const data = await res.json();
      if (data.success) { setMessage('Ride updated successfully!'); setEditingId(null); fetchProfileAndRides(); }
  ...
  const handleDeleteRide = async (id) => {
    ...
    try {
      const res = await fetch(`/api/rides/${id}?t=${Date.now()}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) { setMessage('Ride deleted successfully!'); fetchProfileAndRides(); }
      else setMessage(`Error: ${data.error}`);
    } catch (err) { setMessage(`Error: ${err.message}`); }
  };
          ...
          <div className="p-6">
            ...
            {loading ? (
              ...
              <div className="text-center py-8 text-gray-600">
                {activeTab === 'myRides' ? 'No rides offered yet. Start by offering a ride!' : 'No rides available from others.'}
              </div>
                    ...
                    {editingId === ride._id ? (
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <select name="vehicleType" value={editFormData.vehicleType} onChange={handleEditInputChange}
                              ...
                              <option value="Car">Car</option><option value="Micro">Micro</option><option value="Bike">Bike</option>
                            </select>
                          </div>
                          <div>
                        ...
                        <div className="flex gap-3">
                          <button onClick={() => handleUpdateRide(ride._id)} className="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-lg transition">Save Changes</button>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4 text-sm">
                          ...
                          {ride.distanceKm && <div><p className="text-gray-500">Distance</p><p className="font-semibold text-gray-800">{ride.distanceKm} km</p></div>}
                          {ride.duration && <div><p className="text-gray-500">Est. Time</p><p className="font-semibold text-gray-800">{ride.duration}</p></div>}
